...
router.post('/', async (req, res) => {
	...
	try {
		...
		// get artist info
		const responseInfo = await fetch(`http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=${req.body.searchArtist}&api_key=${API_KEY}&format=json`)
		if (!responseInfo.ok) {
		...
		const responseTopTracks = await fetch(`http://ws.audioscrobbler.com/2.0/?method=artist.gettoptracks&artist=${req.body.searchArtist}&api_key=${API_KEY}&limit=20&format=json`)
		...
		const responseTopAlbums = await fetch(`http://ws.audioscrobbler.com/2.0/?method=artist.gettopalbums&artist=${req.body.searchArtist}&api_key=${API_KEY}&limit=5&format=json`)
